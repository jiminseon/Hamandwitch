...
function scene:create( event )
	...
	pf2.x = display.contentCenterX + 740
	...
	pf5.x = display.contentCenterX + 400
	...
	pf6.x = display.contentCenterX + 280
	pf6.y = display.contentCenterY - 250
	...
	seed.y = display.contentCenterY - 40
	...
	local function sensorCollide( self, event )
		...
		if ( collideObject == seed ) then
			print("getSeed")
			display.remove(collideObject)
			transition.moveBy(c_door, {y=-270, time=500})
			getSeed = true
		end
		-- 메모 --
		if ( collideObject == memoIcon ) then
			if ( event.phase == "began" ) then
				print( "memo opened" )
				memo = display.newImage("image/memo4.png")
		...
		if ( collideObject == door_physics ) then
			if (isDoorOpened == true) then
				...
				timer.performWithDelay( 300, function()
					...
					composer.gotoScene("stage4_clear")
				end
				)
			end
		end
	...
	local function stop(event)
		local pauseBG = display.newImage("image/pause2-1.png")
