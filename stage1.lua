...
function scene:create( event )
	...
	local textbox = display.newImage("image/MainTextbox.png")
	...
	left.x = display.contentCenterX - 170
	left.y = display.contentCenterY + 240
	sceneGroup:insert(left)

	local right = display.newImage("image/right.png")
	right.x = display.contentCenterX + 20
	right.y = display.contentCenterY + 240
	sceneGroup:insert(right)

	local space = display.newImage("image/space.png")
	space.x = display.contentCenterX + 300
	space.y = display.contentCenterY + 240
	sceneGroup:insert(space)

	local story = display.newText("왼쪽 :\t\t\t\t\t\t 오른쪽 :\t\t\t\t\t\t 점프 :", display.contentCenterX + 230, display.contentCenterY + 370, textbox.x + 80, 300, "font/BMJUA_ttf.ttf")
	...
	local function Next(event)
		...
		display.remove(space)
		display.remove(story)
	end
	...
	-- platForm --
	local pf1 = display.newImage("image/pf_2.png")
	pf1.x = display.contentCenterX + 150
	pf1.y = display.contentCenterY + 350
	...
	pf2.x = display.contentCenterX - 200
	...
	key.x = display.contentCenterX + 750
	key.y = display.contentCenterY + 370
	...
	local function sensorCollide( self, event )
		...
		if ( collideObject == memoIcon ) then
			if ( event.phase == "began" ) then
				print( "memo opened" )
				memo = display.newImage("image/memo1.png")
		...
		if ( collideObject == door_physics ) then
			if (isDoorOpened == true) then
				...
				timer.performWithDelay( 300, function()
					...
					composer.gotoScene("stage1_clear")
				end
				)
			end
		end
	...
	local function stop(event)
		local pauseBG = display.newImage("image/pause1-1.png")
