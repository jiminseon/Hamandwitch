...
function scene:create( event )
	...
	local background = display.newImage("image/ending_6.png")
	...
	local story = display.newText("뭐야 설마 이 낡은 책밖에 없는거야…? 반짝반짝 빛나는 금덩이도 없어…?", display.contentCenterX , display.contentCenterY + 370,textbox.x + 80, 300, "font/BMJUA_ttf.ttf")
	story:setFillColor(0.3)
	story.size = 40
	sceneGroup:insert(story)

	local function Next(event)
		composer.gotoScene("ending_7")
	end
