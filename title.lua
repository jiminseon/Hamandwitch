...
function scene:create( event )
	...
	local title = display.newImage("image/title.png")
	title.x = display.contentCenterX
	title.y = display.contentCenterY 
	sceneGroup:insert(title)

	local start = display.newImage("image/GameStart.png")
	...
	local function GameStart(event)
		composer.gotoScene("opening_1", options)
	end

	start:addEventListener("tap",GameStart)
